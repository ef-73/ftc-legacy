...
public class ChassisSystem {
    ...
    public static volatile DcMotorEx leftFrontDrive = null;
    public static volatile DcMotorEx rightFrontDrive = null;
    public static volatile DcMotorEx leftRearDrive = null;
    public static volatile DcMotorEx rightRearDrive = null;

    private static Telemetry telemetry;
    public static volatile double speedFactor = 1.0;

    private double lastLFPower = 0;
    private double lastLRPower = 0;
    private double lastRFPower = 0;
    private double lastRRPower = 0;
    private double rampRate = 0.2;   // 20ms control rate,

    public ChassisSystem(LinearOpMode mainTask){
        this.mainTask = mainTask;

        leftFrontDrive = mainTask.hardwareMap.get(DcMotorEx.class, "lf");
        rightFrontDrive = mainTask.hardwareMap.get(DcMotorEx.class, "rf");
        leftRearDrive = mainTask.hardwareMap.get(DcMotorEx.class, "lr");
        rightRearDrive = mainTask.hardwareMap.get(DcMotorEx.class, "rr");
    ...
    public void driveRobotRamp(double lfPower, double lrPower, double rfPower, double rrPower)
    {
        double magnitude = Math.abs(lfPower);
        magnitude = Math.max(magnitude, Math.abs(lrPower));
        magnitude = Math.max(magnitude, Math.abs(rfPower));
        ...
        lastLFPower = leftFrontDrive.getPower();
        if (Math.abs(lfPower - lastLFPower) > rampRate){
            lfPower = lastLFPower + Math.signum(lfPower - lastLFPower) * rampRate;
        }
        lastLRPower = leftRearDrive.getPower();
        if (Math.abs(lrPower - lastLRPower) > rampRate){
            lrPower = lastLRPower + Math.signum(lrPower - lastLRPower) * rampRate;
        }

        lastRFPower = rightFrontDrive.getPower();
        if (Math.abs(rfPower - lastRFPower) > rampRate){
            rfPower = lastRFPower + Math.signum(rfPower - lastRFPower) * rampRate;
        }
        lastRRPower = rightRearDrive.getPower();
        if (Math.abs(rrPower - lastRRPower) > rampRate){
            rrPower = lastRRPower + Math.signum(rrPower - lastRRPower) * rampRate;
        }
    ...
    {
        if(mainTask.gamepad1.right_bumper ){
            speedFactor = 1.0;    // 1.0
        }
        else{
            speedFactor = 0.4;
        ...
        double leftStickXPos = -mainTask.gamepad1.left_stick_x * speedFactor;
        double leftStickYPos = -mainTask.gamepad1.left_stick_y * speedFactor;
        double rightStickXPos = -mainTask.gamepad1.right_stick_x * speedFactor;
      ...
      //  driveRobotRamp(lfPower, lrPower, rfPower,rrPower);
    }
    ...
    // set the robot side movement keep heading
