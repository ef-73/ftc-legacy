...
public class MecanumDriveLib {
    ...
    public static final double MAX_VEL =  1.8;  // 2.1865 * 0.8 =1.75 2.1865 * 0.9 =1.96785 m/s, some lose here, need test, For 435 RPM motor, 435 /60 * PI * WHEEL_D(0.096) = 2.1865 * 0.8 =1.75
    public static final double MAX_ROTATION_VEL = 2 * Math.PI ; // randian /s ; 1 second full circle
    ...
    public static double P2PX_KP = 3.0; //1.5;  1.6 -- 435 RPM  3.0 - 312RPM
    public static double P2PX_KD = 15.0;  //10 -- 435 RPM  15 - 312 RPM
    public static double P2PX_KI = 0.1;
    public static double p2pXKp = P2PX_KP;
    public static double p2pXKd = P2PX_KD;
    public static double p2pXKi = P2PX_KI;


    // when only drive along X, could red
    public static double P2PY_KP = 4.5;  // 3.2 435 RPM  5.0 - 312RPM
    public static double P2PY_KD = 10.0;  // 8.0 -- 435 RPM 10- 312 RPM
    public static double P2PY_KI = 0.3;
    public static double p2pYKp = P2PY_KP;
    public static double p2pYKd = P2PY_KD;
    public static double p2pYKi = P2PY_KI;


    public static double P2PTURN_KP = 0.008;
    public static double P2PTURN_KD = 0.1;
    public static double P2PTURN_KI = 0.001;
    public static double p2pTurnKp = P2PTURN_KP;
    public static double p2pTurnKi = P2PTURN_KI;
    public static double p2pTurnKd = P2PTURN_KD;


    public static double turnKp = 0.015;
    public static double turnKi = 0.1;
    public static double turnKd = 0.08;
    ...
    private static double holdPosXKp = 0.2;
    private static double holdPosXKd = 2.0;
    private static double holdPosXKi = 0.05;
    private static double holdPosXMaxKi = 0.3;

    private static double holdPosYKp = 0.4;
    private static double holdPosYKd = 1.0;
    private static double holdPosYKi = 0.1;
    private static double holdPosYMaxKi = 0.5;

    private static double holdPosTurnKp = 0.01;
    private static double holdPosTurnKd = 0.1;
    private static double holdPosTurnKi = 0.05;
    private static double holdPosTurnMaxKi = 0.3;
    ...
    private static double ACCEL_TOFULL_TIME = 200;  //accelerate to full power time  500 32sec
    ...
    public static volatile double powerX ;
    public static volatile double powerY = 0;


    MecanumDriveLib(LinearOpMode mainTask,Odometry odom, ChassisSystem chassis)
    {
    ...
    public void p2pDrive(RobotPosition targetPos, double p2pVel, double turnVel, double startVel, double disTolerance, double angleTolerance, double endVel,boolean stopAtEnd, int timeOut)
    {
        if (mainTask.opModeIsActive()) {

            p2pTaskFlag = true;
            p2PDiveThread = new P2PDiveThread(targetPos, p2pVel, turnVel, startVel, disTolerance, angleTolerance, endVel, stopAtEnd, timeOut);
    ...
    private static class P2PDiveThread extends Thread {
        ...
        private static volatile double endVelX;
        private static volatile double endVelY;
        ...
        private static double endVelX_r = 0;
        private static double endVelY_r = 0;
        private static double x_Tolorence = 0.02;
        private static double y_Tolorence = 0.02;
        private static double min_Tolorence = 0.01;

        private static volatile double startVel = 0.0;


        P2PDiveThread(RobotPosition targetPos, double p2pVel, double turnVel, double startVel, double disTolerance, double angleToLerance, double endVel,boolean stopAtEnd, int timeOut)
        {
            ...
            endVelX = Math.abs(endvel * Math.cos(Math.toRadians(targetPos.angle)));
            endVelY = Math.abs(endvel * Math.sin(Math.toRadians(targetPos.angle)));
            double tolorenceK = 0.2;
            min_Tolorence = disTolerance * 0.5 < 0.01 ? 0.01 : disTolerance * 0.5;


            x_Tolorence = endVelX * tolorenceK;
            x_Tolorence = x_Tolorence < min_Tolorence ? min_Tolorence : x_Tolorence;

            y_Tolorence = endVelY * tolorenceK;
            y_Tolorence = y_Tolorence < min_Tolorence ? min_Tolorence : y_Tolorence;
        ...
        public void run() {
            ...
            do{
                try {
                    vMax =  startVel + runtime.milliseconds() / ACCEL_TOFULL_TIME;
                    vMax = vMax > p2pVel ? Math.abs(p2pVel) : vMax;

                    nowPos = odometry.getRobotPosition();

                    endVelX_r = Math.abs(Math.cos(Math.toRadians(nowPos.angle)) * endVelX + Math.sin(Math.toRadians(nowPos.angle)) * endVelY);
                    endVelY_r = Math.abs(-Math.sin(Math.toRadians(nowPos.angle)) * endVelX + Math.cos(Math.toRadians(nowPos.angle)) * endVelY);
...
*/
                    ...
                    deltaXG = targetPos.x - nowPos.x;
                    deltaYG = targetPos.y - nowPos.y;

                    deltaX = Math.cos(Math.toRadians(nowPos.angle)) * deltaXG + Math.sin(Math.toRadians(nowPos.angle)) * deltaYG;
                    deltaY = -Math.sin(Math.toRadians(nowPos.angle)) * deltaXG + Math.cos(Math.toRadians(nowPos.angle)) * deltaYG;


                    if (Math.abs(deltaX) > Math.abs(x_Tolorence))
                    {
                        powerX =  fPID.calculate(deltaX);

                        powerX = Math.abs(powerX) > Math.abs(vMax) ? Math.signum(powerX) * Math.abs(vMax) : powerX;
                        powerX = Math.abs(powerX) < Math.abs(endVelX_r) ? Math.signum(powerX) * Math.abs(endVelX_r) : powerX;
                        ...
                        powerX = Math.abs(powerX) > 1.0 ? Math.signum(powerX) * 1.0 : powerX;
                    }
                    else
                    {
                        powerX = 0;
                    ...
                    if (Math.abs(deltaY) > Math.abs(y_Tolorence))
                    {
                        powerY = sPID.calculate(deltaY);
                        powerY = Math.abs(powerY) > Math.abs(vMax) ? Math.signum(powerY) * Math.abs(vMax) : powerY;

                        powerY = Math.abs(powerY) < Math.abs(endVelY_r) ? Math.signum(powerY) * Math.abs(endVelY_r) : powerY;
                        ...
                        powerY = Math.abs(powerY) > 1.0? Math.signum(powerY) * 1.0 : powerY;
                    ...
                    if (this.stopAtEnd)
                    {
                        ...
                        if (powerX == 0 && powerY == 0 && Math.abs(angleError) < angleTolerance)
                        {
                            p2pTaskFlag = false;
                        }
...
*/
    ...
    public void resetP2PPID(){
        ...
        p2pTurnKp = P2PTURN_KP;
        p2pTurnKi = P2PTURN_KI;
        p2pTurnKd = P2PTURN_KD;
    ...
    public void setP2PYPid(double kp, double ki, double kd){
        p2pYKp = kp;
        p2pYKi = ki;
        p2pYKd = kd;
    }

    public void setP2PXPid(double kp, double ki, double kd){
        p2pXKp = kp;
        p2pXKi = ki;
        p2pXKd = kd;
    }
